...
In this blog, I am going to calculate the probability of several questions about the Braves and the Yankees in the World Series.
...
\centering
 \begin{tabular}{||c c c ||} 
 \hline
Parameter	 & Explaination  \\ [0.5ex] 
 \hline\hline
PB	 & In any given game, the probability that the Braves win	 \\ 
PY = 1 - PB	 & In any given game, the probability that the Yankees win	 \\ [1ex] 
 \hline
 \end{tabular}
\end{table}
...
Create a function to calculate the probability of win. 
Win is defined as win 4 times in 7 games.
```{r}
...
Obviously, when the PB is 0.55, the probability that Braves win the World Series is 0.6082878.
...
First, we need to generate the series of PB and the probability results.
```{r}
...
In order to intepret the relationship between PB and the probability that the Braves win, we can draw a graph for them.
...
As same as the first question, the PB needs to be 0.55. And now the game series length is not a certain. Definitely, the series length should be an odd number.
```{r}
PB <- 0.55
...
Now the PB is not defined again, so we should assume the x could be any number between 0.51 to 1.

First, we need to generate the series of PB and a series to save the length results given different PB. But the way, we also need a series of the possible series length we will test. Now the ceiling is 9999. If it’s not enough, we can set a bigger limitation.
```{r}
...
In order to calculate the probabilty that the Braves win the WS, we need a new function with 2 input, because both of the series length and PB are variables.
```{r}
...
We have already get the shortest series length given different PB. Let’s draw the figure to show the relationship between them.
...
in this graph, when PB is increasing, the shortest series length, when the probability that the Braves win the World Series is more than 0.8, is approching 1. When the PB is bigger than 0.8, the shortest series length is 1.
...
### 5.Calculate P( PB=0.55|Braves lose 3 games before winning a 4th game) under the assumption that either PB=0.55 or PB=0.45. Explain your solution.
 ...
 P(A) = P(PB=0.55), P(B) = P(Braves lose 3 games before winning a 4th game). As a result, P( PB=0.55|Braves lose 3 games before winning a 4th game) = P(Braves lose 3 games before winning a 4th game|PB=0.55)×P(PB=0.55)÷P(Braves lose 3 games before winning a 4th game).
